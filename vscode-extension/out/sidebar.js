...
class SidebarProvider {
    constructor(ctx, mcp, getProject) {
        this.ctx = ctx;
        this.mcp = mcp;
        this.getProject = getProject;
        this.memories = [];
        this.stats = { total: 0, pinned: 0, links: 0, healthScore: 0, hot: 0 };
        this.loading = false;
        this.sessionText = "";
    }
    ...
    setSession(text) { this.sessionText = text; this.render(); }
    ...
    async handleMessage(msg) {
        switch (msg.command) {
            ...
            case "addMemory": {
                ...
                if (!sel?.trim()) {
                    vscode.window.showWarningMessage("Selecione um texto no editor para salvar como memória.");
                    break;
                }
            ...
            case "openBrainDoctor": {
                const url = vscode.workspace.getConfiguration("memory-mcp").get("serverUrl") ?? "";
    ...
    buildHtml() {
        ...
        const s = this.stats;
        const pill = (v, color) => `<span style="background:${color}22;color:${color};border:1px solid ${color}44;border-radius:4px;padding:1px 6px;font-size:10px;font-weight:700">${v}</span>`;
